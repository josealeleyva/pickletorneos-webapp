...
@section('page-title', 'Crear Cancha')
...
<div class="max-w-2xl mx-auto">
    ...
    <nav class="mb-4 sm:mb-6">
        <ol class="flex items-center space-x-2 text-xs sm:text-sm text-gray-600">
            ...
            <li>
                <a href="{{ route('complejos.canchas.index', $complejo) }}" class="hover:text-brand-600">{{ $complejo->nombre }}</a>
            </li>
            <li>
                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path>
            ...
            <li class="text-gray-900 font-medium">Nueva Cancha</li>
        </ol>
    </nav>
    ...
    <div class="bg-white rounded-lg shadow-sm p-4 sm:p-6 md:p-8">
        ...
        <div class="mb-6 pb-6 border-b border-gray-200">
            <p class="text-xs sm:text-sm text-gray-600">Agregando cancha a:</p>
            <p class="text-lg font-semibold text-gray-900">{{ $complejo->nombre }}</p>
        </div>

        <form action="{{ route('complejos.canchas.store', $complejo) }}" method="POST">
            @csrf
            ...
            <div class="mb-6">
                ...
                <input
                    ...
                    id="numero"
                    name="numero"
                    value="{{ old('numero', $complejo->canchas()->max('numero') + 1 ?? 1) }}"
                    required
                    min="1"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-transparent @error('numero') border-red-500 @enderror"
                    placeholder="1"
                >
                @error('numero')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
