<div>
    @php
        ...
        $campeon = null;
        if ($llaveFinal && $llaveFinal->partido && $llaveFinal->partido->estado === 'finalizado') {
    ...
    @if($campeon)
        <div class="flex items-center gap-3 bg-gradient-to-r from-yellow-400 to-yellow-500 text-gray-900 px-4 py-2 rounded-lg shadow-lg mb-4">
    ...
    @if($llavesPorRonda->isEmpty())
        <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-center">
            <p class="text-sm text-yellow-800">No hay llaves generadas para esta categoría.</p>
        </div>
    @else
        <!-- Accordion por ronda -->
        <div class="space-y-3">
            @foreach($rondas as $rondaNombre)
                ...
                <div class="border border-gray-200 rounded-lg overflow-hidden">
                    <button type="button"
                            class="ronda-button w-full px-4 py-3 bg-gray-50 text-left flex items-center justify-between"
                            data-ronda="ronda-{{ Str::slug($rondaNombre) }}-{{ $categoria->id }}">
                        <div>
                            <span class="font-semibold text-gray-800">{{ $rondaNombre }}</span>
                            <span class="ml-2 text-xs text-gray-500">({{ $llaves->count() }})</span>
                        </div>
                    ...
                    <div class="ronda-content {{ $loop->first ? '' : 'hidden' }} p-3 space-y-3" id="ronda-{{ Str::slug($rondaNombre) }}-{{ $categoria->id }}">
        ...
        <!--@if($llavesPorRonda->has('3er Puesto'))
            <div class="mt-6 pt-4 border-t border-gray-200">
                <h3 class="font-semibold text-gray-800 mb-3 text-sm">3er y 4to Puesto</h3>
                @php
                ...
                @if($tercerPuesto)
                    <div class="bg-white border border-gray-300 rounded-lg overflow-hidden">
                        <div class="bg-gray-50 px-3 py-2 border-b border-gray-200">
                            <div class="text-xs text-gray-600 space-y-1">
                                <div class="font-semibold text-gray-700">3er Puesto</div>

                                @if($tercerPuesto->partido)
                                    @if($tercerPuesto->partido->fecha_hora)
                                        <div class="flex items-center">
                                            <svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                                            </svg>
                                            {{ $tercerPuesto->partido->fecha_hora->format('d/m/Y H:i') }}
                                        </div>
                                    @endif

                                    @if($tercerPuesto->partido->cancha)
                                        <div class="flex items-center">
                                            <svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path>
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                            </svg>
                                            {{ $tercerPuesto->partido->cancha->complejo->nombre }}, {{ $tercerPuesto->partido->cancha->nombre }}
                                        </div>
                                    @endif
                                @endif
                            </div>
                        ...
                        <div class="divide-y divide-gray-200">
                            <div class="px-3 py-2.5 {{ $tercerPuesto->partido && $tercerPuesto->partido->equipo_ganador_id === $tercerPuesto->equipo1_id ? 'bg-green-50 font-semibold' : '' }}">
                                ...
                                @else
                                    <span class="text-sm text-gray-500 italic">Pendiente</span>
                                @endif
                            </div>

                            <div class="px-3 py-2.5 {{ $tercerPuesto->partido && $tercerPuesto->partido->equipo_ganador_id === $tercerPuesto->equipo2_id ? 'bg-green-50 font-semibold' : '' }}">
...
<script>
    // Accordion para rondas en mobile
    document.querySelectorAll('.ronda-button').forEach(button => {
        button.addEventListener('click', function() {
            const targetId = this.dataset.ronda;
